...
#### **하지만 생각보다 이렇게 단순하지 않습니다..**

위에서 설명드린 것만 보면 메우 단순합니다. <u>하지만, 이 방식으로 분석한 결과를 인과 효과라고 하기 위해서는 반드시 필요한 조건이 있습니다. 바로 두 집단(실험군, 대조군) 사이에 "영웅/전설 획득"을 제외하고 다른 성질이 동일해야한다는 조건입니다.</u> "영웅/전설 획득이 플레이 변화의 원인일까?"를 확인하기 위해서는, 플레이 변화의 원인이 되는 다른 것들을 동일하게 유지하고 영웅/전설 획득만 바꾸었을 때 플레이 변화 차이가 발생하는지 측정해야하기 때문입니다. 

* 예를 들어, 어떤 변수 A가 플레이 변화에 영향을 미치는 원인이고 실험군과 대조군 두 집단에서 A가 매우 다른 상태라고 해봅시다.
* 만약 두 집단의 플레이 변화 차이가 존재한다라는 결론이 나왔을 때, 이것이 영웅/전설 획득에 때문에 일어난 결과라고 할 수 있을까요? 
* 아닙니다. 이것은 변수 A에 의해 발생한 결과일 수 있습니다. 

이렇듯 단순히 두 집단의 플레이 변화를 비교한다면 영웅/전설 획득이 플레이 변화에 원인이 아니었음에도 우리는 원인이다라고 잘못된 인과 효과를 주장할 수도 있습니다. 

때문에 본 분석에서 실험군, 대조군 간에 성질을 동일하게 맞춰주는 "인과 추론"이라는 절차가 매우 중요하였고 그 과정을 먼저 소개드리려 합니다.
...
<p align="center"><img src = "/assets/works/class_get_causal_analysis/image1_1.png" width="50%" /><br>[그림2-1] 원인 변수와 결과 변수에 동시에 영향을 주는 교란 변수</p>
...
그렇다면 교란 변수에 의한 "상관 관계"를 제거하고 우리가 원하는 인과 효과를 추정하려면 어떻게 해야할까요? 바로 교란 변수를 통제한 후 두 집단의 "플레이 변화"를 비교하면 됩니다. 교란 변수를 통제한다는 것은 교란 변수의 값이 같은 샘플끼리 묶어서 비교한다는 의미이고, 실제 분석 시에는 추정 모델에 교란 변수를 통제 변수로 추가하여 적용할 수 있습니다. 

*교란 변수를 통제를 해야하는 이유는 Appendix에 수식으로 설명되어 있으니 참고하시면 좋을 것 같습니다.* 
...
<p align="center"><img src = "/assets/works/class_get_causal_analysis/image2_2.png" width="80%" /><br>[그림3-2] collider를 통제하면 일부 샘플만 분석에 사용됩니다(collider bias)</p>
